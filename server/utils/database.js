...
(()=>{
  ...
  const debug = require('debug')('meeting:db');

  const Database = require('better-sqlite3');
  const dbfilename = path.resolve(__dirname, '../',process.env.MEETING_DB_DIR, process.env.MEETING_DB);
  let db;
  try {
    db = new Database(dbfilename, {fileMustExist:true, timeout: parseInt(process.env.MEETING_DB_BUSY,10)});
  } catch(e) {
    if (e.code === 'SQLITE_CANTOPEN') {
      ...
      try {
        ...
        db = new Database(dbfilename, { fileMustExist: false, timeout: parseInt(process.env.MEETING_DB_BUSY,10) });
