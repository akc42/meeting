...
(function() {
  ...
  const debug = require('debug')('meeting:host:rooms');
  const db = require('@akc42/server-utils/database');

  module.exports = async function(user, hoster, params, responder) {
    debug('new request from user', user );
    const rooms = db.prepare('SELECt * FROM room WHERE host = ?').all(hoster.name);
    responder.addSection('rooms', rooms);
    debug('done');
  };
})();